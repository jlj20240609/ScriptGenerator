...
子命令：
  capture   把当前前台窗口存为目标：页面=窗口客户区双截图；部件=手动框选(3次Enter)
            或自动(--text 指定部件文字)。产物 JSON+PNG 存入 --dir。
  run       对 targets 目录里的目标循环定位+点击：随机移动窗口 → 页面模板定位 →
            部件三级定位(①UIA树 ②OCR文本/模板相似度 ③页面内坐标) → 点击 →
            校验(点击点仍含文字 + 与已知位移真值比对)。逐次写 data/trials.jsonl。
  selftest  合成图上验证定位器（无需真实桌面）。

用法示例：
  python smoke/m0_trial.py capture --cls web-login --name 登录页 --text 登录按钮
  python smoke/m0_trial.py run --dir smoke/targets --count 20 --move
  python smoke/m0_trial.py selftest
"""
import argparse
import ctypes
import random
...
def client_rect_of(hwnd):
    import win32gui
    cr = win32gui.GetClientRect(hwnd)
    pt = win32gui.ClientToScreen(hwnd, (0, 0))
    return (pt[0], pt[1], cr[2], cr[3])
...
def do_capture(args):
    ...
    hwnd = fg["hwnd"]
    if not hwnd or not fg["title"]:
        raise SystemExit("没有可用的前台窗口")
    crect = client_rect_of(hwnd)
    print("前台窗口: %r hwnd=%d 客户区=%s" % (fg["title"], hwnd, crect))
    m0lib.bring_to_foreground(hwnd)
    time.sleep(0.5)
    crect = client_rect_of(hwnd)
    page_bgr = m0lib.grab_screen(crect)
    if args.manual:
        p1 = None
        p2 = None
        ...
        # 中途窗口可能被移动：以第二次抓客户区核对
        crect2 = client_rect_of(hwnd)
        drift = max(abs(crect2[i] - crect[i]) for i in range(4))
        if drift > 10:
        ...
        click_in_page = (click_pt[0] - crect[0], click_pt[1] - crect[1])
        wrect_in_page = (wrect[0] - crect[0], wrect[1] - crect[1], wrect[2], wrect[3])
    else:
        text = args.text or ""
        if not text:
            raise SystemExit("自动模式需要 --text 指定部件文字")
        crop = page_bgr
        f = m0lib.find_text_ocr(crop, text)
        if not f["ok"]:
            print("窗口内未找到文字 '%s'（OCR 结果数=%d），可尝试 --manual" % (text, f["ocr_count"]))
            # 打印看到的文字供纠错
            r = m0lib.ocr_run(crop)
            print("窗口内 OCR 文字:", r["txts"][:12])
            raise SystemExit(1)
        bx, by, bw, bh = f["box"]
    ...
    ocr_text = cands[0] if cands else (args.text or "")
    # UIA 命中（若有）
    uia = m0lib.uia_walk_find_text(hwnd, ocr_text, max_nodes=2500, max_ms=3000)
    uia_name = uia.get("name", "") if uia.get("ok") else ""
    # 记录显示器信息
    mm = monitor_rect(1)
    try:
    ...
    target = m0lib.Target(
        ...
        page={"file": "%s_page.png" % args.name, "rect": list(crect),
              "window_title": fg["title"], "window_class": fg["class"], "hwnd": int(hwnd)},
    ...
    print("  类别=%s ocr_text=%r uia_name=%r delta=%s" %
          (target.cls, target.ocr_text, target.uia_name, target.click["delta_from_widget_center"]))
...
def locate_widget_chain(target, page_live_bgr, page_rect, hwnd=None):
    ...
    L1 UIA 树（窗口句柄内找同名控件）→ L2 OCR 文本 / 模板相似度 → L3 页面内坐标映射。
    """
    ...
    if hwnd and target.ocr_text:
        ...
        if u.get("ok") and u.get("rect"):
            r = u["rect"]
            l1 = {"ok": True, "method": "uia", "confidence": 1.0, "box": r,
                  "center": (r[0] + r[2] // 2, r[1] + r[3] // 2),
                  "elapsed_ms": (time.perf_counter() - t0) * 1000, "name": u["name"]}
        else:
    ...
    if target.ocr_text:
        t0 = time.perf_counter()
        f = m0lib.find_text_ocr(page_live_bgr, target.ocr_text)
        if f["ok"]:
            bx, by, bw, bh = f["box"]
            ...
            l2o = {"ok": False, "elapsed_ms": f["elapsed_ms"]}
    detail["l2_ocr"] = l2o
    # --- L2 模板相似度（部件图 vs 页面图） ---
    l2t = {"ok": False}
    ...
    while attempt < args.count:
        for tpath in targets:
            ...
            if hwnd:
                ...
                try:
                    if not win32gui.IsWindow(hwnd):
                        print("[%s] 窗口句柄失效，尝试按标题找回" % tpath.stem)
                        hwnd = win32gui.FindWindow(None, target.page.get("window_title", ""))
                        if not hwnd:
            ...
            if args.move and hwnd:
                # 移到屏幕内随机可见位置（真实“窗口挪了位置”场景）
                import win32gui
                win32api = m0lib.win32("win32api", "win32api")
                win32con = m0lib.win32("win32con", "win32con")
                try:
                    ...
                    if maxx > 40 and maxy > 40:
                        ...
                        win32api.SetWindowPos(hwnd, win32con.HWND_TOP, nx, ny, 0, 0,
                                              win32con.SWP_NOSIZE)
                        moved = True
                        time.sleep(0.5)
            ...
            if hwnd:
                ...
                try:
                    import win32gui
                    raise_ok = win32gui.GetForegroundWindow() == hwnd
                except Exception:
                    raise_ok = False
            crect = client_rect_of(hwnd) if hwnd else None
            page_expected = crect
            screen_bgr = m0lib.grab_screen()
            t0 = time.perf_counter()
            page = locate_page_live(target, screen_bgr, page_expected)
            row = {"type": "trial", "target_id": target.target_id, "cls": target.cls,
                   "attempt": attempt, "moved": moved, "raised": raise_ok,
            ...
            if not page["ok"]:
                ...
                print("[%03d] %-10s page FAIL score=%.2f" % (attempt, target.cls, page["score"]))
                time.sleep(args.interval)
                continue
            pr = page["rect"]
            page_live = screen_bgr[pr[1]:pr[1] + pr[3], pr[0]:pr[0] + pr[2]].copy()
            chosen, detail = locate_widget_chain(target, page_live, pr, hwnd=hwnd)
            row["widget"]["detail"] = {
                "l1": {k: detail["l1"][k] for k in ("ok", "method", "elapsed_ms")
                       if k in detail["l1"]},
                "l2_ocr": {k: detail["l2_ocr"][k] for k in ("ok", "method", "confidence", "elapsed_ms")
            ...
            # 真值：窗口客户区原点 + 部件中心在页内偏移（移动窗口时内容整体平移）
            truth = None
            dev_px = None
            if page_expected:
            ...
            time.sleep(0.25)
            # 校验：点击点周边小块 OCR 是否仍含目标文字（文本类部件）；纯图标用模板分代替
            ver = {"method": "ocr_patch", "ok": False}
            if target.ocr_text:
                patch = m0lib.grab_screen((cx - 70, cy - 28, 140, 56))
                f = m0lib.find_text_ocr(patch, target.ocr_text, upsample=2)
